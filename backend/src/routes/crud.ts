...
// Whitelist de tabelas permitidas via CRUD genérico
const ALLOWED_TABLES = new Set([
  'profiles', 'drivers', 'vehicles', 'journeys', 'journey_legs',
  'expenses', 'fuel_expenses', 'revenue', 'accounts_payable',
  'expense_categories', 'revenue_categories', 'parties',
  'vehicle_maintenances', 'maintenance_parts', 'maintenance_schedules',
  'workshops', 'tire_assets', 'tire_logs', 'tire_history',
  'bank_transactions', 'bank_reconciliations',
  'cte_documents', 'cte_settings', 'cte_series',
  'mdfe_documents', 'mdfe_cte_links',
  'driver_messages', 'driver_performance_history',
  'vehicle_couplings', 'vehicle_coupling_items', 'saved_couplings',
  'journey_checklists', 'incidents', 'announcements',
  'freight_requests', 'freight_rates', 'freight_pricing_settings',
  'financial_accounts', 'financial_reserves', 'financial_reserve_entries',
  'gestor_permissions', 'system_alerts', 'system_settings',
]);

// Tabelas que NÃO têm company_id (globais)
const GLOBAL_TABLES = new Set(['subscription_plans', 'system_settings']);

function buildWhereClause(queryParams: Record<string, any>, companyId: string, tableName: string): { sql: string; params: any[] } {
  ...
  // Suporte a filtros simples ?field=value (além dos reservados)
...
router.get('/:table', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const { sql: whereSql, params } = buildWhereClause(req.query as any, companyId, table);
...
router.post('/:table', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const inserted = await queryOne(`SELECT * FROM \`${table}\` WHERE id = ?`, [data.id]);
    res.status(201).json(inserted);
  } catch (err: any) {
    console.error(`POST /${table} error:`, err);
    res.status(500).json({ error: err.message || 'Erro ao inserir' });
  }
});
...
router.put('/:table/:id', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = await execute(
      ...
      GLOBAL_TABLES.has(table) ? [...Object.values(data), id] : values
    );
