...
const PlanFeaturesContext = createContext<PlanFeatures>(defaultFeatures);
...
export function PlanFeaturesProvider({ children }: { children: ReactNode }) {
  ...
  const fetchPlanFeatures = useCallback(async () => {
    if (!currentCompany?.id) {
      console.log('[PlanFeatures] No currentCompany, skipping fetch');
      setFeatures(prev => ({ ...prev, isLoading: false }));
      return;
    }

    console.log('[PlanFeatures] Fetching features for company:', currentCompany.id);
    ...
    try {
      ...
      if (companyError) {
        console.error('[PlanFeatures] Error fetching company:', companyError);
        setFeatures(prev => ({ ...prev, isLoading: false }));
        return;
      }

      if (!companyData?.subscription_plan_id) {
        console.log('[PlanFeatures] No subscription_plan_id found');
        setFeatures(prev => ({ ...prev, isLoading: false }));
        return;
      }

      console.log('[PlanFeatures] subscription_plan_id:', companyData.subscription_plan_id);
      ...
      if (planError) {
        console.error('[PlanFeatures] Error fetching plan:', planError);
        setFeatures(prev => ({ ...prev, isLoading: false }));
        return;
      }

      if (!planData) {
        console.log('[PlanFeatures] No plan data found');
        setFeatures(prev => ({ ...prev, isLoading: false }));
        return;
      }

      const plan = planData as unknown as SubscriptionPlanData;
      console.log('[PlanFeatures] Plan loaded:', plan.name, {
      ...
      console.log('[PlanFeatures] Features updated:', newFeatures);
      setFeatures(newFeatures);
    } catch (error) {
      console.error('[PlanFeatures] Error fetching plan features:', error);
      setFeatures(prev => ({ ...prev, isLoading: false }));
    }
  }, [currentCompany?.id]);
  ...
  useEffect(() => {
    ...
    console.log('[PlanFeatures] Setting up realtime subscription for company:', currentCompany.id);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[PlanFeatures] Realtime update received:', payload);
          fetchPlanFeatures();
        }
      )
      .subscribe((status) => {
        console.log('[PlanFeatures] Realtime subscription status:', status);
      });

    return () => {
      console.log('[PlanFeatures] Cleaning up realtime subscription');
...
export function usePlanFeaturesContext(): PlanFeatures {
